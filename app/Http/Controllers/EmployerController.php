...
class EmployerController extends Controller
{
    ...
    // عرض جميع طلبات أصحاب العمل المعلقة
    ...
    // لتقديم طلب من دور المستخدم إلى دور صاحب العمل:
    public function apply(Request $request)
    {
        ...
        // التحقق مما إذا كان المستخدم قد قدم طلبًا سابقًا للترقية
        ...
        // إضافة مستندات التحقق إلى قاعدة البيانات
        if ($request->hasFile('verification_documents')) {
            $filePath = $request->file('verification_documents')->store('public/verification_documents');//path of stoage docs
    ...
    // لإدارة الطلبات التي يحتاجها المستخدمون ليصبحوا أصحاب عمل؛ رفض أو قبول
    public function handleRequest(Request $request, $user_id)
    {
        ...
        if ($action === 'approve') {
            ...
            // حذف بيانات صاحب العمل من قاعدة البيانات
            if ($employer) {
                $employer->delete();
            }
            // تحديث حالة المستخدم والدور
